...
local StartBtn = createAnimatedButton("START STEAL", UDim2.new(0.05, 0, 0.70, 0))

-----------------------------------------------------------
-- LÓGICA DE TELEPORTE SAFE
-----------------------------------------------------------
...
SetupBtn.MouseButton1Click:Connect(function()
    ...
    if root then
        ...
        SetupBtn.Text = "POSIÇÃO DEFINIDA!"
        SetupBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
        task.wait(1)
        SetupBtn.Text = "SETUP POSITION"
        SetupBtn.TextColor3 = Color3.fromRGB(0, 170, 255)
    end
end)
...
StartBtn.MouseButton1Click:Connect(function()
    ...
    if root and savedPos then
        -- Teleporte suave com pequena altura para evitar bugs de colisão
        root.AssemblyLinearVelocity = Vector3.new(0,0,0)
        root.CFrame = savedPos + Vector3.new(0, 0.5, 0)
    elseif not savedPos then
        StartBtn.Text = "NADA SALVO!"
        task.wait(1)
        StartBtn.Text = "START STEAL"
    end
end)

-- Animação de Entrada do Menu
MainFrame.Position = UDim2.new(0.5, -130, -0.5, 0)
MainFrame:TweenPosition(UDim2.new(0.5, -130, 0.4, 0), "Out", "Back", 0.8, true)
